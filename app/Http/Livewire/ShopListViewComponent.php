...
class ShopListViewComponent extends Component
{
    ...
    {
        if($this->sorting == '1') {
            $products = Product::whereBetween('price', [$this->min_price, $this->max_price])->where('featured', '1')->orderBy('created_at','DESC')->paginate($this->pagesize)->where('product_type',$this->new)->where('product_type',$this->used)->where('product_type',$this->refurnished);
        }
        ...
        return view('livewire.shop-list-view-component', ['products' => $products])->layout("layouts.base");
    }
}